...
def search_activity():
    ...
    if current_user.is_authenticated:
        ...
        return render_template(
            "activity.html", today_date=today_date, load_map=False, actiu=True
        )
    else:
...
def search_activity_post():
    ...
    new_activity = Activitat(
        ...
        keywords=getKeywords(
            request.form.get("titol"), corpus_keys=all_keywords
        ),  # example: walk,beach,tour
    )
...
def form_test(id_activitat):
    ...
    return render_template(
        "activity_detail.html", this_activity=this_activity, actiu=True
    )
